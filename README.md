...
系统支持大厂标准的 Docker 容器化多阶段构建与版本化交付体系：

```bash
# 1. 复制生产环境变量并按需调整
cp .env.production.example .env

# 2. 一键拉起生产容器（数据持久化至 ./data）
docker compose up -d
...
更多关于**一键打包版本离线包**、**虚拟机平滑无损更新**与**自动冷备**，请参阅：
- 🐳 [Docker生产级容器化部署与版本更新指南](docs/Docker生产级容器化部署与版本更新指南.md)

### 4. 传统物理机部署方案

系统亦支持传统单机部署、虚拟机挂载及云服务器生产级部署（PM2 + Nginx 反代 + SSL 证书）。详细方案可参阅：
...
项目包含完备的架构设计、自控力工程推导、演进复盘与运维部署文档，存放于 `docs/` 目录：

| 文档名称 | 核心内容 |
| :--- | :--- |
| 📂 [各业务模块详细设计草案套件 (00~08)](docs/modules/00-模块总览与设计规范.md) | **【模块套件】** 涵盖 8 大核心模块详细职责、源文件、DDL 契约与状态机的全套 Draft 设计文档 |
| 🧩 [全系统核心业务模块与架构联动全景白皮书](docs/Sacred_Focus_全系统核心业务模块与架构联动全景白皮书.md) | **【权威】** 系统 8 大核心业务模块划分、源码路径清单与跨模块协同流转全景文档 |
| 🛡️ [全系统深度审查报告 (2026-09-14)](docs/Sacred_Focus_全系统深度审查报告_2026-09-14.md) | **【最新】** 全系统安全防御、容器部署、自控状态机与性能损耗全维度深度审查报告 |
| 🐳 [Docker生产级容器化部署与版本更新指南](docs/Docker生产级容器化部署与版本更新指南.md) | **【推荐】** 多阶段构建、版本化 Tar/Registry 交付、平滑更新与数据零丢失备份方案 |
| 📘 [全系统深度架构与设计实现全景白皮书](docs/Sacred_Focus全系统深度架构与设计实现全景白皮书.md) | 涵盖 CTDP / RSIP 哲学推导、微观与宏观状态机、SQLite WAL 十大表规范与全系统架构设计 |
| 📜 [国策树业务基线与四维规范字典](docs/国策树_v1.0.md) | 初始国策树全集定义、触发时机、分组规划与四维规范卡权威参考 |
| 🚀 [开发历程与里程碑全景总结 (M1~M11)](docs/开发历程与里程碑全景总结.md) | 记录 M1~M11 全阶段架构演进、移动端重构、Docker 版本化更新与代码审查全量修复 |
| 📋 [代码审查权威报告 (CODE_REVIEW)](docs/CODE_REVIEW.md) | 系统性深度代码审查报告，涵盖 18 项安全防御、并发事务、状态机严密性与架构缺陷详析 |
| 🛠️ [代码审查问题修复全景记录](docs/代码审查问题修复记录.md) | 18 项代码审查问题逐项深度溯源、技术方案重构、原子提交与全链路测试验证实录 |
| 🔍 [系统全链路验证与体验调优审查报告](docs/系统全链路验证与体验调优审查报告.md) | 物理时钟保真、破坏性弹窗根治、深浅双模高对比度、容器滚动与全链路健康度 |
| 🛡️ [生产级安全加固与鉴权防护方案](docs/Sacred_Focus全系统生产级安全加固与鉴权防护方案.md) | JWT 双轨鉴权、多级分级限流、可信内网智能免检、预热备容灾与数据防御 |
| ☁️ [从虚拟机到公网云端全流程部署实战教程](docs/Sacred_Focus从VMware虚拟机到云服务器全流程部署实战教程.md) | Linux (Ubuntu)、PM2 守护、Nginx 反向代理、SSL 证书配置与跨设备备份恢复实战教程 |
